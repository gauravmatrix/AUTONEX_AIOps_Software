...
const Dashboard = () => {
  ...
  return (
    <div className="p-6 space-y-6 md:p-8 lg:p-12" data-testid="dashboard">
      {/* Header */}
      <motion.div
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 md:gap-6">
        ...
        <div className="p-6 lg:col-span-2 glass-card rounded-xl">
        ...
        {/* AI Status Card */}
        <div className="p-6 space-y-6 glass-card rounded-xl">
          <div>
            <h2 className="mb-4 text-2xl font-bold tracking-tight">AI Engine</h2>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 border rounded-lg bg-primary/10 border-primary/20">
                ...
                <span className="px-2 py-1 font-mono text-xs text-green-400 rounded bg-green-500/20">
          ...
          {/* Quick Actions */}
          <div>
            <h3 className="mb-3 font-mono text-sm tracking-wider uppercase text-muted-foreground">
              Quick Actions
            </h3>
            <div className="space-y-2">
              ...
              <a href="/anomalies" className="block">
                <button className="w-full px-4 py-2 font-medium transition-all duration-300 rounded-md bg-white/5 hover:bg-white/10">
                  View Anomalies
                </button>
              </a>
            </div>
      ...
      {recentMetrics.length > 0 && (
        <div className="p-6 glass-card rounded-xl">
          <h2 className="mb-6 text-2xl font-bold tracking-tight">System Metrics Overview</h2>
...
const StatCard = ({ icon: Icon, label, value, trend, testId }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        <p className="mb-1 text-3xl font-black tracking-tight">{value}</p>
        <p className="font-mono text-xs tracking-wider uppercase opacity-80">{label}</p>
      </div>
    </motion.div>
  );
};
...
const ServiceHealthCard = ({ service }) => {
  ...
  return (
    <motion.div
      ...
      className="p-4 transition-all border rounded-lg bg-white/5 border-white/10 hover:border-white/20"
    ...
    >
      ...
      <div className="grid grid-cols-4 gap-2">
        <div>
          <p className="font-mono text-xs text-muted-foreground">CPU</p>
          <p className="font-mono text-sm">{service.cpu?.toFixed(0)}%</p>
        </div>
        <div>
          <p className="font-mono text-xs text-muted-foreground">MEM</p>
          <p className="font-mono text-sm">{service.memory?.toFixed(0)}%</p>
        </div>
        <div>
          <p className="font-mono text-xs text-muted-foreground">LAT</p>
          <p className="font-mono text-sm">{service.latency?.toFixed(0)}ms</p>
        </div>
        <div>
          <p className="font-mono text-xs text-muted-foreground">ERR</p>
          <p className="font-mono text-sm">{service.error_rate?.toFixed(1)}%</p>
        </div>
      </div>
    </motion.div>
  );
